...
using JLD2, CairoMakie, Colors, LaTeXStrings, FHist
...
function PlotterTrace(
    path_to_file_with_binned_data::String,
    path_to_png_with_trace::String,
    bin_size::Float64,
    folder::String,
    file::String
    )
...
function PlotterHistogram(
    path_to_file_with_binned_data::String,
    path_to_png_with_histogram::String,
    bin_size::Float64,
    folder::String,
    file::String
    )
    ...
    hist_obj = Hist1D(
        intensity;
        binedges = minimum(intensity) : maximum(intensity)
    ) |> normalize
    fig, ax, p = plot(
        hist_obj,
        figure = (
        ...
        axis = (
            xlabel = "Intensity, [count/$(UInt32(bin_size*1000)) ms]",
            ylabel = "Occurence",
            limits = (0, maximum(intensity), 0, 1.1*maximum(bincounts(hist_obj))),
        ),
            color = :black,
            gap = 0
    )
...
function PlotterFLID(
    path_to_file_with_FLID_data::String,
    path_to_png_with_FLID::String,
    folder::String,
    file::String
    )
    FLID_data = JLD2.load_object(
        path_to_file_with_FLID_data
    )
